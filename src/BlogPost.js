            ...
            return (
                  <Col size={4}>
                    ...
                    {item._embedded['wp:term'][1].map((name, idx) => {
                      return (
                        <span className="categories" key={idx}>{ (idx ? ', ' : '') + name.name }</span>
                      );
                    })}
                    </div>
                    <img key={item.index} src={item.featured_media} />
                    <a href={item.link}><h3 key={item.index} className="postTitle">{item.title.rendered}</h3></a>
                    <p className="postMeta">
                    {item._embedded.author.map((author, idx) => {
                      return (
                        <p key={idx}>By <a href={author.link}>{author.name}</a> on {Moment(item.date).format("Do MMMM YYYY")}</p>
                      );
                    })}
                      </p>
                    ...
                    {item._embedded['wp:term'][0].map((name, idx) => {
                      return (
                        <p className="categories" key={idx}>{name.name}</p>
                      );
                    })}
                    </div>
                    </Card>
